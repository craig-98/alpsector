...
import { User, Mail, Phone, Shield, Bell, Lock, LogOut, Camera, CheckCircle, Clock, XCircle } from 'lucide-react';
...
export default function ProfilePage() {
  ...
  const fetchProfile = async () => {
    try {
      ...
      const actualProfile = Array.isArray(profileData) ? profileData[0] : profileData;
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-blue-50">
        <div className="text-center p-8">
          ...
          <p className="text-xl font-bold text-gray-700">Loading profile...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-dark-grey-50 via-dark-grey-100 to-white text-black font-bold pb-24">
      ...
      <main className="px-6 pb-12">
        <div className="max-w-7xl mx-auto space-y-8">
          {/* Profile Card */}
          <motion.div className="bg-white rounded-3xl p-8 shadow-lg" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <Button onClick={handleSaveProfile} disabled={saving} className="mt-6 bg-emerald-600">
              {saving ? 'Saving...' : 'Save Changes'}
            </Button>
          </motion.div>

          {/* KYC Status */}
          <motion.div className="bg-white rounded-3xl p-8 shadow-lg" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
            ...
            <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-xl">
              {profile?.kycStatus === 'verified' ? (
                ...
                <>
                  ...
                  <Button className="ml-auto bg-emerald-600">Complete KYC</Button>
                </>
              )}
            </div>
          </motion.div>
          ...
          {/* Security */}
          <motion.div className="bg-white rounded-3xl p-8 shadow-lg" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
          ...
          {/* Notifications */}
          <motion.div className="bg-white rounded-3xl p-8 shadow-lg" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
            <Button onClick={handleLogout} variant="destructive" className="w-full py-6">
